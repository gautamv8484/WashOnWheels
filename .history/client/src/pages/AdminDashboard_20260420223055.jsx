  ...
  const fetchAllData = async () => {
    ...
    try {
      const [bookingsRes, subsRes, contactsRes] = await Promise.all([
        axios.get('http://localhost:5000/api/bookings').catch(() => ({ data: { bookings: [] } })),
        axios.get('http://localhost:5000/api/subscriptions').catch(() => ({ data: { subscriptions: [] } })),
        axios.get('http://localhost:5000/api/contact').catch(() => ({ data: { contacts: [] } }))
      ])
